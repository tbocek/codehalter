...
- Check `${PIPESTATUS[@]}` for long-running pipelines if you need to
...
1. Open the script and look up line N — that's the failing command, but
   the diagnostic (e.g. `package site is not in std`) was printed by a
   sub-process a few lines earlier in the output.
2. Re-run just that line via `run_command` in isolation. This gets you
   the clean error without the wrapper's "exit 1" footer drowning it.
3. If the failing line invokes another bash script (`bash foo.sh ARG`),
   re-run it with `bash -x foo.sh ARG`. The `-x` trace prefixes each
   command with `+` — the last `+` line before the error tells you which
   inner line broke.
