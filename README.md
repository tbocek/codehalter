...
- **Self-tuning prompts** (`/improve`): the agent re-reads its own session logs and proposes edits to the prompt and SKILL files under `.codehalter/` — and *measures* each candidate before proposing it. `probe_statement` answers the same question on your main model with and without a statement in the skill, so a line that changes nothing gets dropped on evidence rather than on opinion. Every edit arrives as a card you Apply or Skip; the analysis itself can be routed to a stronger, slower model with `purpose = "improve"`.
...
- **Native MCP client**: `.codehalter/mcp.toml` spawns each `[[server]]` as a long-lived child, registers its tools under `<name>__<tool>`, and reconciles on every prompt; Go projects are offered gopls via a one-click setup card.
