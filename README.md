...
- **Stack-aware skills** — auto-detected stacks (Go, TypeScript, JavaScript, Java, Bash, devcontainer) seed `SKILL-*.md` files in `.codehalter/`; they're concatenated into the system prompt on the first turn so they ride along in cached history. Drop your own SKILL-*.md to add conventions for any language.
