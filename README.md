...
- **Native MCP client**: `.codehalter/mcp.toml` spawns each `[[server]]` as a long-lived child, registers its tools under `<name>__<tool>`, and reconciles at turn boundaries; Go projects are offered gopls via a one-click setup card.
