...
- **Built-in tools**: `read_file`, `continue_read` (paging through a large read), `write_file`, `edit_file`, `run_command` (in-container only; listing is `git ls-files` and searching is `grep -n -C` there, each doing in one call what a tool would, so there is neither; a command still running after two minutes is not killed but continues as a background job, the model gets what it printed so far and its exit later, and a turn that ends on `respond` while such a job runs is parked, not finished: the user can interject, and the job's exit or its `wake_after` resumes the turn where it stood; a handed-over command with no output and no log growth for ten minutes is killed as hung), `run_background` (a dev server, or a long experiment: the process outlives the tool call and the turn, the chat stays usable, and codehalter reports the exit code and last output by itself when it finishes: to the model before its next step when a turn is running, so it can start a test suite, keep editing, and find the result waiting; as one line after the turn ends when it finished later than that, never an interruption; and when idle the agent looks at the result and tells you; a foreground `sleep` is refused while a job runs, since the job wakes the model itself, and a job that does not exit can be given `wake_after` seconds to be looked at with its log tail while it still runs; a turn that ends with a new job still running says so once, and that the work resumes when it exits; jobs die with the session), `view_image` (when the active model has vision), `ask_user` (a pick-one list, a free-text box, or both, sent as an ACP elicitation form), `web_search`, `web_read` (the page's text, paged from a cached body), `respond`, `submit_plan`. An output too large for the context is cut to head and tail with a hint on how to narrow the call; there is no tool to page a cached output. Plus any MCP-server tool, registered as `<server-name>__<tool>`.
...
- **Spec loop (`/spec`)**: plain `/spec` in a project that has no loop yet asks three questions, each with up to three options read off the project (which directory holds the spec; where to build, the entry page's own answer first and then any directory that already holds a manifest; and with what, the page's answer, what that directory already is, and the keyword table's reading); the answers go to `.codehalter/spec.toml`, and editing that file is how they change. `/spec stop`, sent while the loop runs (**Send Now** in Zed), ends it at the next round boundary: the round in flight finishes and commits, the status is printed, and a later `/spec` resumes from the ledger. Either way it implements a specification item by item until every requirement has a passing test. The bookkeeping is code, not model memory:
  ...
  - **Done:** an item counts as done when a test under the output dir names it (`f2_3_…`, or the id in a comment) and the project's test command passes. An item that starts with a user action also needs a test that fires the real widget and checks the effect: the logic test proves the rule, the widget test proves the wire, so a program of tested rules that no click reaches cannot pass. Codehalter checks both after every round and commits each item that passes.
  ...
  - **The spec is the master:** every finished item is recorded with a fingerprint of the spec text it was built from, plus when it was recorded and by which codehalter (tag, build date and commit); an item a test already covered is adopted the same way, minus the commit. Edit that section later and the next run redoes the item, showing the model the `git diff` of the edit rather than the section again; delete the section and the run offers to delete its code and tests too. A section that moves to another file or is retitled keeps its record instead of looking like one deletion plus one new item.
  - **Too big for a plan:** when a chat request is many rounds of work (a page to build, "make it all work"), the planner hands it to the loop instead of planning it: in a project built with `/spec` it names the spec items concerned and codehalter offers to reopen them; in a project without a spec it writes one (markdown, one section per requirement) into `spec/`, shows it, and offers to run `/spec` on it, so the spec stays something the user has read before forty rounds are tested against it.
  - **Final pass:** once every item is covered, one more round builds the program the way a user would, runs it, compares each screen with the spec's image, looks for the seams between items, and writes `<out dir>/README.md` with how to build, run and test it and what is not done; it is committed when the test command passes, recorded in the ledger, and redone when the item or blocked count moves. A failed round never counts as done just because its test exists: the item stays open until the suite passes.
  - **Stop and resume:** the loop stops after three blocked items in a row. `/spec` resumes and `/spec status` shows coverage per chapter plus anything that changed in the spec since; state lives in `.codehalter/spec.toml`. `/spec redo` alone has the model audit the program against the spec, build, run and look at every screen, and name the finished items that do not deliver; the list is shown and the loop starts on it at once, one item per round. `/spec redo <item id or spec file>` sends named items back: they leave the ledger, their tests no longer count until a round passes, the round is told why, and the loop starts on them at once, so work that passed its tests but missed the spec (a screen without its widgets) can be redone without editing the spec.
- **Web tools**: `web_search` (DuckDuckGo) returns a list of result titles/URLs/snippets for the LLM to triage; `web_read` then opens a chosen URL in headless Firefox and returns its text, paged from a cached body. Restricted to the planning phase so execution stays offline.
