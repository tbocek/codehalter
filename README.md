...
- **Reusable prompt macros**: type `/<name> <args>` to expand a `TEMPLATE-<name>.md` into a full prompt. Ships **`/grill-me`** (the agent interviews you about a plan until the whole design tree is resolved), **`/commit`** (drafts a commit message from your diff plus the conversation's intent, then commits — never pushes unless asked), and **`/clean`** (deletes session log files from `.codehalter/`); drop your own `TEMPLATE-*.md` for project- or team-specific workflows.
...
- **Per-model skill pruning** ([`crafter/`](crafter/README.md)): a research subproject that *measures* which SKILL statements a given model actually needs — segment each skill into atomic claims, probe every claim with the whole skill loaded vs. nothing, and drop the lines the model already follows unprompted — emitting a pruned, per-model skill file. Different models (and finetunes) keep different lines; smaller skills mean less prefill on every turn. See [crafter/README.md](crafter/README.md).
...
- **Bring-your-own MCP**: `.codehalter/mcp.toml` declares each `[[server]]` as a child process to spawn (`name`, `command`, `args`, `env`); tools it advertises are registered as `<name>__<tool>`. The seeded file declares no servers; a project with a `go.mod` instead gets a single setup card offering to install gopls (if missing) and wire it in one turn (`gopls mcp` exposes `go_symbols`, `go_references`, `go_definition`, `go_hover`, …). Reconciled on every prompt, add or remove a `[[server]]` between turns and the change applies without restarting; failed startups surface as a red tool-call card and mtime-gate so they don't re-emit until you edit the file.
...
- **Native MCP client**: `.codehalter/mcp.toml` spawns each `[[server]]` as a long-lived child, registers its tools under `<name>__<tool>`, and reconciles on every prompt; Go projects are offered gopls via a one-click setup card. Dual-era: each server is probed via `server/discover` and spoken to with the stateless 2026-07-28 revision ("MCP 2", per-request `_meta` + mirrored `Mcp-Method`/`Mcp-Name`/`Mcp-Param-*` headers, no sessions) when it supports it, with automatic fallback to the 2025-06-18 initialize/session handshake for everything else.
