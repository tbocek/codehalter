...
- **Project onboarding**: detects your stack and task runners, seeds the skills for your container and task runner, and bootstraps empty repositories. When a tool is missing, it offers a one-click **"fix it?" card** rather than just failing.
...
Plus the skills: `SKILL-base.md` (container, install order, toolchains, formatter config), one `SKILL-<distro>.md` for the container's OS, `SKILL-justfile.md` / `SKILL-makefile.md` when the project has one, and `SKILL-layout.md` (check a rendered page with `screenshot`, measure before editing) when it has stylesheets or HTML templates. There are no per-language skills: a capable model knows its languages, and the skills cover what it cannot know. The runner and layout skills are deferred until the session first touches a matching file (or concatenated into the system prompt up front with `skills = "inline"`).
