...
- **Reusable prompt macros**: type `/<name> <args>` to expand a `TEMPLATE-<name>.md` into a full prompt. Ships **`/grill-me`** (the agent interviews you about a plan until the whole design tree is resolved) and **`/clean`** (deletes session log files from `.codehalter/`); drop your own `TEMPLATE-*.md` for project- or team-specific workflows.
