...
func (a *agent) verify(ctx context.Context, sid string, fallbackConn *LLMConnection, plan *planResult, res toolLoopResult) (toolLoopResult, *verifyResult, error) {
	...
	if err != nil {
		...
		a.sendUpdate(ctx, sid, messageChunk{Kind: KindAgentMessage, Content: ContentBlock{Type: "text", Text: "⚠ Verification skipped: " + err.Error() + "\n"}})
		return res, &verifyResult{Success: true}, nil
	}
...
func (a *agent) runTaskCycle(
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		if looped {
			a.sendUpdate(ctx, sid, messageChunk{Kind: KindAgentMessage, Content: ContentBlock{Type: "text", Text: fmt.Sprintf("⚠ Same verification failure as a prior attempt — retrying would loop. Giving up. Last issues:\n%s\n", strings.Join(vr.Issues, "\n"))}})
			break
		}
		seenBags = append(seenBags, currentBag)

		if attempt == maxAttempts-1 {
			a.sendUpdate(ctx, sid, messageChunk{Kind: KindAgentMessage, Content: ContentBlock{Type: "text", Text: fmt.Sprintf("⚠ Verification still failing after %d attempts — giving up. Last issues:\n%s\n", maxAttempts, strings.Join(vr.Issues, "\n"))}})
			break
		}
