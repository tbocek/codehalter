...
- **Edit the generic `SKILL-<x>.md` only.** The per-model copies under
  `.codehalter/skills/<variant>/` are GENERATED by the skill crafter and will be
  re-measured from the generic skill — hand edits there are dead ends (the apply
  step rejects such paths anyway).
- **One behavior per bullet, self-contained.** The crafter later segments every
  statement and probes it per model in isolation, so a proposed statement must
  stand alone: name the tool/language, no pronouns referring to neighbors, no
