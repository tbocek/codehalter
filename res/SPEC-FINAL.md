...
3. Start the program the way a user would and look at it: the entry point with `--help`, then for a desktop app every screen through the `snapshot` recipe, at rest and after its main actions (`--do <widget-name>`), then `screenshot` each image and compare it with the spec's image of that screen (widgets, their order, labels and tooltips; not pixels). A screen that does not match, or an item whose code exists but cannot be reached from the UI or the command line, is a gap: fix it when it is small, otherwise list it.
...
5. Write `{{out_dir}}/README.md` (create it, or update it if the project has one; short and true): what this program is, in one paragraph from the spec; how to build it; how to run it, with the exact commands, any environment the container needed (such as `GSK_RENDERER=cairo`), and where a sample project or fixture is (a GUI cannot be run in this container: give the user the Wayland container recipe from the base skill, adapted to this project's image and packages); how to run the tests; what the spec covers; and what is not done: the blocked items below, and every gap you found and did not fix.
